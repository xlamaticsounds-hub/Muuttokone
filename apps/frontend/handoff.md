...
- Goals
  ...
  - Keep Sanity for blog (for now).
  - No cache layer initially.
  - Minimal CRM in Directus; custom CRM at `crm.muuttokone.fi` only if needed later.
...
graph TD
  ...
  subgraph Home Server (Docker)
    ...
    ADMIN --> FS[(Filesystem /data/uploads)]
    API --> FS
...
## Minimal data model (Directus collections)
...
- users/roles: Directus auth for admin access
...
- API
  ...
  - DIRECTUS_STATIC_TOKEN (service account or admin token)
...
- Directus
  ...
  - STORAGE_LOCAL_ROOT (e.g., /data/uploads)
  - PUBLIC_URL (https://mgmt.muuttokone.fi)
...
- Folders you created:
  ...
  - `mgmt.muuttokone.fi` — Directus container and config
  - `crm.muuttokone.fi` — reserved for future custom CRM

- Shared infra:
  - Reverse proxy (Traefik/Caddy) with routes:
    - muuttokone.fi → FE
    - api.muuttokone.fi → API
    - mgmt.muuttokone.fi → Directus
    - crm.muuttokone.fi → CRM (optional)
  - Postgres single instance + backups
  - Directus volume for local file storage (`/data/uploads`)

- Networking:
  - Each service on a common Docker network; only reverse proxy exposes ports 80/443.
