...
import type { ReceiptLineItem, ReceiptPdfProps } from '@/lib/pdf/receipt-pdf-document';

// Oma kopio lib/pdf/receipt-pdf-document.tsx:n rakenteesta — sama syy kuin
// server/pdf/invoice-pdf.tsx:ssä: tiedostojärjestelmäpolku toimii vain palvelimella
// (renderToBuffer), kun taas selaimen PDFDownloadLink tarvitsee julkisen URL:n eikä
// pääse process.cwd()-polkuihin käsiksi.
...
function ReceiptPdfDocumentServer(props: ReceiptPdfProps) {
  const {
    receiptNumber,
    receiptDate,
    customerName,
...
export async function renderReceiptPdf(props: ReceiptPdfProps): Promise<Buffer> {
  return renderToBuffer(<ReceiptPdfDocumentServer {...props} />);
}
