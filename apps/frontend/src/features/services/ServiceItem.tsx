...
export default function ServiceItem({ service, index = 0 }: ServiceItemProps) {
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border border-stroke/10 bg-white p-8 text-center shadow-sm ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl dark:border-stroke/10 dark:bg-[#0f1115] dark:ring-white/5 ${
        service.bgClass || ""
      }`}
    >
      <div className="relative z-10 flex flex-col items-center">
        <div className="mb-6 flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl bg-primary/10 lg:h-20 lg:w-20">
          <Image
            src={iconSrc}
            alt={service.title}
            width={56}
      ...
      <div
        aria-hidden
        className="absolute -right-4 -top-6 h-24 w-24 rounded-full bg-primary/5 blur-md transition-all duration-300 group-hover:bg-primary/10"
      />
      <div
        aria-hidden
