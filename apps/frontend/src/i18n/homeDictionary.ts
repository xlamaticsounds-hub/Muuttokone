...
export const ourStoryDictionary: Record<string, string> = {
  ...
  'Muuttokone syntyi, kun päätimme kaverin kanssa ottaa asiat omiin käsiimme — Suomen työtilanne ei tarjonnut meille eikä kavereillemme sitä mitä halusimme, joten loimme työpaikat itse. Mukaan on sittemmin liittynyt niin kavereita kuin muitakin tekijöitä. Halusimme tuoda perinteiseen muuttoalaan jotain uutta: täyden läpinäkyvyyden ja aidon, rehellisen tekemisen ilman turhia vääntöjä. Tiedämme, että muutto on jo valmiiksi stressaavaa — siksi asiakkaamme ei tarvitse miettiä, osaavatko muuttomiehet hommansa tai yllättääkö lasku suurempana kuin sovittiin. Muutto on meille vasta alku — tähtäämme kasvuun kohti isompaa logistiikka- ja kuljetusalan toimijaa.':
    "Muuttokone was born when a friend and I decided to take things into our own hands — the job market in Finland wasn't offering us or our friends what we wanted, so we created the jobs ourselves. Since then, friends and others have joined in too. We wanted to bring something new to the traditional moving industry: full transparency and honest, genuine work without unnecessary hassle. We know moving is already stressful enough — that's why our customers shouldn't have to wonder whether the movers know what they're doing, or whether the bill turns out bigger than agreed. Moving is just the beginning for us — our goal is to grow into a bigger logistics and transport company.",
};
